...
export default function RewardsScreen() {
  ...
  const renderKidProgress = (kid: Member) => {
    const topReward = activeRewards.length > 0 
      ? activeRewards.reduce((min, r) => r.starsCost < min.starsCost ? r : min)
      : null;
    
    const progress = topReward ? Math.min(100, (kid.starsTotal / topReward.starsCost) * 100) : 0;
    
    return (
      <View key={kid.id} style={styles.kidProgressCard} data-testid={`kid-progress-${kid.id}`}>
        <View style={styles.kidAvatar}>
          <Text style={styles.kidAvatarInitial}>{kid.name.charAt(0).toUpperCase()}</Text>
        </View>
        <View style={styles.kidInfo}>
          <Text style={styles.kidName}>{kid.name}</Text>
          <View style={styles.starsRow}>
            <Ionicons name="star" size={16} color={colors.secondary} />
            <Text style={styles.kidStars}>{kid.starsTotal} stars</Text>
          </View>
          {topReward && (
          ...
          {topReward && (
            <Text style={styles.nextRewardText}>
              {kid.starsTotal >= topReward.starsCost 
                ? `Ready for: ${topReward.title}` 
                : `${topReward.starsCost - kid.starsTotal} more for ${topReward.title}`}
            </Text>
          )}
        </View>
      </View>
